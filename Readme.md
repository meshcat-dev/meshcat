...
When preparing a pull request to land on master, if possible on your computer please run `rebuild/rebuild.sh` to hermetically regenerate the `dist` files. The very first time you run it, it might take a little time to download all of the relevant files. Subsequent runs should be much faster (~10 seconds).
