...
class Background extends THREE.Object3D {
    ...
    update(scene, is_visible, is_perspective) {
        ...
        // In immersive AR mode, fall back to the default three.js background
        // (null for both background and environment).
        let cam_key = is_perspective ? "round" : "flat";
        if (this.use_ar_background) {
...
class Viewer {
    ...
    set_camera(obj) {
        if (this.webxr_session_active) {
            console.warn("Can't set camera during an active session.");
            return;
        }
        this.camera = obj;
        ...
        // Disable the WebXR button if the camera is orthographic.
        if (!this.is_perspective && this.xr_button != null) {
            this.xr_button.disableButton();
            console.warn("WebXR Button is disabled, because the camera is in Orthographic view.");
        }
    ...
    enable_webxr(mode = "ar") {
        ...
        if (this.camera.isOrthographicCamera) {
            console.error("Can not enable webxr because the camera is in Orthographic mode.");
            return;
        }
        if (mode == "vr") {
