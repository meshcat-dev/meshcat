...
import * as THREE from 'three' // TODO: why is .default needed?
...
function merge_geometries (object, preserve_materials = false) {
  const materials = []
  const geometries = []
  const root_transform = object.matrix.clone()
  function collectGeometries (node, parent_transform) {
    const transform = parent_transform.clone().multiply(node.matrix)
    if (node.type === 'Mesh') {
...
function handle_special_geometry (geom) {
  ...
  if (geom.type == '_meshfile_geometry') {
    if (geom.format == 'obj') {
      ...
      const obj = loader.parse(geom.data + '\n')
      const loaded_geom = merge_geometries(obj)
      loaded_geom.uuid = geom.uuid
      return loaded_geom
    } else if (geom.format == 'dae') {
      const loader = new ColladaLoader()
      const obj = loader.parse(geom.data)
      const result = merge_geometries(obj.scene)
      result.uuid = geom.uuid
      return result
    } else if (geom.format == 'stl') {
      const loader = new STLLoader()
      const loaded_geom = loader.parse(geom.data.buffer)
      loaded_geom.uuid = geom.uuid
      return loaded_geom
    } else {
...
class ExtensibleObjectLoader extends THREE.ObjectLoader {
  delegate (special_handler, base_handler, json, additional_objects) {
    ...
    const remaining_json = []
    for (const data of json) {
      const x = special_handler(data)
      if (x !== null) {
        result[x.uuid] = x
      } else {
  ...
  parseObject (json, geometries, materials) {
    if (json.type == '_meshfile_object') {
      ...
      const path = (json.url === undefined) ? undefined : THREE.LoaderUtils.extractUrlBase(json.url)
      ...
      if (json.format == 'obj') {
        ...
        if (json.mtl_library) {
          ...
          const mtl_parse_result = mtl_loader.parse(json.mtl_library + '\n', '')
          const materials = MtlObjBridge.addMaterialsFromMtlLoader(mtl_parse_result)
          loader.setMaterials(materials)
          this.onTextureLoad()
        }
        const obj = loader.parse(json.data + '\n', path)
        ...
        const obj = loader.parse(json.data, path)
...
class Background extends THREE.Object3D {
  ...
  update (scene, is_visible, is_perspective) {
    ...
    const using_gradient = !is_perspective ||
    ...
    if (is_visible && using_gradient &&
      ...
      const t = [this.state.top_color.r, this.state.top_color.g,
        this.state.top_color.b]
      const b = [this.state.bottom_color.r, this.state.bottom_color.g,
    ...
    // rendering the same.
    const cam_key = is_perspective ? 'round' : 'flat'
    scene.background =
...
class SceneNode {
  ...
  add_child (object) {
    ...
    const node = new SceneNode(object, f, this.on_update)
  ...
  create_controls () {
    ...
    if (this.object.isLight) {
      const intensity_controller = this.folder.add(this.object, 'intensity')
      ...
      if (this.object.castShadow !== undefined) {
        const cast_shadow_controller = this.folder.add(this.object, 'castShadow')
        ...
        if (this.object.shadow !== undefined) {
          // Light source radius
          const radius_controller = this.folder.add(this.object.shadow, 'radius').min(0).step(0.05).max(3.0)
      ...
      if (this.object.distance !== undefined) {
        const distance_controller = this.folder.add(this.object, 'distance').min(0).step(0.1).max(100.0)
        distance_controller.onChange(() => this.on_update())
        this.controllers.push(distance_controller)
      }
    }
    if (this.object.isCamera) {
      const controller = this.folder.add(this.object, 'zoom').min(0).step(0.1)
      controller.onChange(() => {
    ...
    if (this.object.isEnvironment) {
      const intensity_controller = this.folder.add(this.object, 'intensity').min(0).step(0.1).max(100)
      intensity_controller.onChange(() => this.on_update())
      this.controllers.push(intensity_controller)
    }
    if (this.object.isBackground) {
      ...
      const top_controller = this.folder.addColor(this.object, 'top_color')
      top_controller.onChange(() => this.on_update())
      this.controllers.push(top_controller)

      const bottom_controller = this.folder.addColor(this.object, 'bottom_color')
      bottom_controller.onChange(() => this.on_update())
      this.controllers.push(bottom_controller)

      const map_controller = this.folder.add(this.object, 'render_environment_map')
...
class Animator {
  ...
  load (animations, options) {
    ...
    this.folder.open()
    const folder = this.folder.addFolder('default')
    folder.open()
    ...
    const recording_folder = folder.addFolder('Recording')
    ...
    for (const animation of animations) {
      const target = this.viewer.scene_tree.find(animation.path).object
      const clip = THREE.AnimationClip.parse(animation.clip)
      ...
      const action = this.mixer.clipAction(clip, target)
  ...
  update () {
    if (this.playing) {
      ...
      this.viewer.set_dirty()
      if (this.duration != 0) {
        const current_time = this.actions.reduce((acc, action) => {
...
function load_env_texture (path, background, scene, is_visible, is_perspective) {
  // let has_error = false;
  const texture = new THREE.TextureLoader().load(path, undefined, undefined, () => {
...
function wait_for_property (target_node, property, predicate) {
  return new Promise(resolve => {
    const callback = () => {
      ...
      const prop_object = target_node[property]
...
class Viewer {
  ...
  update_background () {
    const bg_parent = this.scene_tree.find(['Background'])
    const bg = this.scene_tree.find(['Background', '<object>'])
    const is_visible = bg_parent.object.visible && bg.object.visible
  ...
  create_default_spot_light () {
    ...
    spot_light.shadow.camera.far = 50.0 // default 500
    spot_light.shadow.bias = -0.001
    return spot_light
  }

  add_default_scene_elements () {
    const spot_light = this.create_default_spot_light()
    ...
    const point_light_px = new THREE.PointLight(0xffffff,
    ...
    point_light_px.shadow.camera.far = 10.0 // default 500
    point_light_px.shadow.bias = -0.001 // Default 0
    this.set_object(['Lights', 'PointLightNegativeX'], point_light_px)

    const point_light_nx = new THREE.PointLight(0xffffff,
      this.upgrade_intensity(0.4))
    ...
    point_light_nx.shadow.camera.far = 10.0 // default 500
    ...
    const fill_light = new THREE.DirectionalLight(0xffffff,
      this.upgrade_intensity(0.4))
  ...
  create_scene_tree () {
    ...
    const scene_folder = this.gui.addFolder('Scene')
    scene_folder.open()
    this.scene_tree = new SceneNode(this.scene, scene_folder, () => this.set_dirty())
    const save_folder = this.gui.addFolder('Save / Load / Capture')
    ...
    const bg_folder = this.scene_tree.find(['Background'])
  ...
  capture_image (w, h) {
    const w_prev = this.dom_element.offsetWidth
    const h_prev = this.dom_element.offsetHeight
  ...
  set_render_callback (callback) {
    const my_callback = eval(callback)
  ...
  set_object_from_json (path, object_json) {
    ...
    // Mesh nodes.
    const meshes_cast_shadows = (node) => {
      if (node.type === 'Mesh') {
  ...
  set_control (name, callback, value, min, max, step,
    ...
    if (value !== undefined) {
      ...
      if (keycode1 != undefined) {
        const keydown_callback = { name, callback: my_callback }
  ...
  set_control_value (name, value, invoke_callback = true) {
    if (name in this.gui_controllers && this.gui_controllers[name] instanceof
  ...
  delete_control (name) {
    ...
    for (const code in this.keydown_callbacks) {
  ...
  handle_command (cmd) {
    if (cmd.type == 'set_transform') {
      ...
      const imgdata = this.capture_image(w, h)
  ...
  handle_command_bytearray (bytearray) {
    const decoded = msgpack.decode(bytearray, { extensionCodec })
  ...
  load_scene_from_json (json) {
    ...
    this.create_scene_tree()
    const cam_node = this.scene_tree.find(['Cameras', 'default', 'rotated', '<object>'])
  ...
  enable_webxr (mode = 'ar') {
    ...
    let original_update_projection_matrix = null
    this.renderer.xr.addEventListener('sessionstart', () => {
      ...
      this.renderer.xr.getSession().requestReferenceSpace('local')
        .then((refSpace) => {
          ...
          const heading_W = new THREE.Vector3(Cz_W.x, 0, Cz_W.z)
          heading_W.normalize()
          const Wz = new THREE.Vector3(0, 0, 1)
          const quat_CW = new THREE.Quaternion()
          ...
          const transform = new XRRigidTransform(p_CW_C, quat_CW)
