...
class SceneNode {
            ...
            try {
                if (!(final_property in target)) {
                    console.warn(`Trying to set the property '${property}'. ` +
                                ...
                                `property '${final_property}'. Creating it with ` +
                                `the value: ${value}`);
                }
    ...
    maybe_recurse_property(parent, child, parent_path) {
        try {
            if (!(child in parent)) {
            console.warn(`When setting a nested property, '${parent_path}' doesn't ` +
                         `have child property '${child}'; adding it as an object.`);
            ...
            console.error("When setting a nested property, the intermediate " +
                          `object '${parent_path}' cannot accept the property ` +
                          `'${child}'. It has type ${typeof(parent)}.` +
                          `\nError message: ${error}.`);
            return null;
        }
        return parent[child];
